...
```bash
uv run python tools/auto_tune_attack.py \
	--backbone resnet50 \
	--checkpoint model/trained_classifier/resnet50/best_classifier.pt \
	--datadir psoriasis_normal \
	--sample-index 0 \
...
默认输出根目录（可通过 `--output-root` 覆盖）：`output/autotune/<backbone>/samoo_autotune_sample_<idx>/autotune_report.json`

## 加入 ImageNet 对照参考集（可选）

如果你想在 `dataset` 中加入一批 ImageNet 图像做“对照参考”（而不是用于当前分类训练），可使用：

```bash
uv run python tools/build_imagenet_reference.py \
	--imagenet-dir /path/to/imagenet/val \
	--output-dir dataset/imagenet_ref_224 \
	--manifest-csv dataset/imagenet_ref_manifest.csv \
	--max-per-class 5 \
	--max-total 500 \
	--image-size 224
```
...
- `--imagenet-dir` 目录下一级子目录应为类别（例如 `n01440764/` 这种结构）。
- 脚本会把图像统一 resize 到 `224x224`，并输出独立 manifest。

注意：

- 生成的 `dataset/imagenet_ref_manifest.csv` 仅用于分析/对照。
- 请不要将该 manifest 直接用于当前 `psoriasis vs normal` 分类训练。

如果你本地没有 ImageNet 数据，可直接下载小规模公开参考数据：

```bash
# 方案 A：下载 CIFAR-10 小样本（推荐先用这个，快）
uv run python tools/download_reference_subset.py \
	--source cifar10 \
	--output-dir dataset/reference_224 \
	--manifest-csv dataset/reference_manifest.csv \
	--max-total 500

# 方案 B：下载 Imagenette（ImageNet 风格公开子集）
uv run python tools/download_reference_subset.py \
	--source imagenette \
	--output-dir dataset/reference_224 \
	--manifest-csv dataset/reference_manifest.csv \
...
- `tools/download_reference_subset.py` 会自动下载并 resize 到 `224x224`。
- 输出 manifest 默认是 `dataset/reference_manifest.csv`。
- 该数据仅建议用于“对照参考/诊断分析”，不要直接并入当前分类训练。
